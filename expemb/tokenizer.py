...
class Tokenizer(ABC):
    ...
    def encode(self, exp: str) -> Tensor: #convert token into index of token, add 'SOE' and 'EOE'
        #print('encode:', exp)
        # indexes = [self.get_index("SOE")] + [self.get_index(comp) for comp in exp.split(" ")] + [self.get_index("EOE")]
        comp_index = []
        for comp in exp.split(" "):
            #print(comp)
            comp_index.append(self.get_index(comp))
        indexes = [self.get_index("SOE")] + comp_index + [self.get_index("EOE")]

        
        return torch.LongTensor(indexes).view(-1)


    def decode(self, encoded: Tensor, ignore_after_eoe: bool = False) -> str: #convert index of token into token
    ...
    def _prefix_to_infix(self, expr):
        ...
        if t in self.get_operators(): #OPERATOR dict, t is an operator
            args = []
            l1 = expr[1:]
        ...
        elif t in self.get_variables() or t in self.get_coefficients() or t in self.get_constants() or t == 'I': #if t is variable 'x' or coefficient 'a1', 'a2'... , or constant "pi", "E", or 'I'
            return t, expr[1:]
